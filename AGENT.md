...
**Фронтенд.** Никаких нативных модальных окон: `window.confirm`, `alert` и `prompt` блокируют
главный поток, а в установленном на домашний экран PWA под iOS диалог может не отрисоваться и
вызов не вернуть управление — страница замирает навсегда. Подтверждения делать компонентом
`ConfirmDialog`. Каждый запрос идёт через `api()` и обязан иметь дедлайн: без него зависшее
соединение не завершает промис и экран, который его ждёт, остаётся в загрузке. Ошибку показывать
через `errorText()`, чтобы таймаут и обрыв связи не выглядели как «что-то пошло не так».
...
- 2026-09-25 — Исправлено зависание админки при отклонении заявки. Причина: `window.confirm` на
  пути отклонения; в PWA на iPhone диалог не отрисовался и не вернул управление, главный поток
  встал, запрос до сервера не дошёл. Заменён на `ConfirmDialog`. Отдельно: `api()` получил
  дедлайны (15 с, 40 с на записи в панель) и общий `errorText()`. `make web` теперь чистит
  `dist` перед сборкой: старые бандлы копились и все попадали в бинарник (2.4 МБ против 416 КБ).
